...
use apollo_federation::query_plan::query_planner::{QueryPlanOptions, QueryPlanner, QueryPlannerConfig};
...
use tower::{load_shed, BoxError, ServiceBuilder};
...
async fn main() {
  ...
  let query_planner = QueryPlanner::new(
    &Supergraph::from_schema(Schema::parse_and_validate(supergraph_sdl, FILENAME).unwrap(), None).unwrap(),
  ...
  serve(TcpListener::bind(addr).await.unwrap(), app).await.unwrap();
}

async fn build_query_plan(State(state): State<SharedState>, Json(payload): Json<BuildQueryPlanRequest>) -> Json<Value> {
  ...
  let document = Valid::assume_valid(
    ExecutableDocument::parse(query_planner.api_schema().schema(), payload.query, "query").unwrap()
  );
